...
export function useMessages({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      // Use getMessagesWithInsights when filtering for messages with AI insights
      if (hasInsights) {
        return sourcesService.getMessagesWithInsights(
          workspaceId,
          page,
          pageSize,
  ...
  const prefetchNextPage = useCallback(() => {
    ...
    queryClient.prefetchQuery({
      ...
      queryFn: () =>
        hasInsights
          ? sourcesService.getMessagesWithInsights(
              workspaceId,
              page + 1,
              pageSize,
              source !== 'all' ? source : undefined,
  ...
  }, [workspaceId, data?.has_next, page, pageSize, source, sortBy, sortOrder, hasInsights, queryClient]);
